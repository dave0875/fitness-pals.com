...
class ActivitySource(Base):
    ...
    decision = Column(String, nullable=False, default="new")  # new | merged | duplicate | conflict
    reason = Column(String, nullable=True)
    chosen_fields = Column(JSON, nullable=True)  # what fields were taken from this source
    raw_payload = Column(JSON, nullable=True)  # optional trimmed payload for transparency
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
...
class IngestDecision(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ingest_run_id = Column(UUID(as_uuid=True), ForeignKey("ingest_runs.id", ondelete="CASCADE"), nullable=False, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    provider = Column(String, nullable=False, index=True)
    ...
    decision = Column(String, nullable=False)  # new | merged | duplicate | conflict | skipped_primary
    reason = Column(String, nullable=True)
