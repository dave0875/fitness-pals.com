...
- `AUTHENTIK_GOOGLE_CLIENT_ID` / `AUTHENTIK_GOOGLE_CLIENT_SECRET`: Google OAuth client used only by Authentik as the upstream identity source. Store these only in the deployment secrets file; the bootstrap never inherits the direct-app fallback pair.
...
Place `AUTHENTIK_GOOGLE_CLIENT_ID` and `AUTHENTIK_GOOGLE_CLIENT_SECRET` in the untracked deployment secrets overlay. They are required for the Authentik Google source. Missing, partial, or placeholder pairs fail before any API mutation, and direct-app fallback credentials are never reused by the broker bootstrap.
